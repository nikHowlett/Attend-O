...
class LoginViewController: UIViewController, UIGestureRecognizerDelegate {
    ...
    override func viewWillAppear(animated: Bool) {
        ...
        if let contents = pageContents {
            ...
            syncronizedNetworkErrorRecieved()
        }
        ...
        let currentUsername = "jill"
        //TSLaunchedUsername
        usernameField.text = currentUsername
        
        if let (savedUsername, savedPassword) = savedCredentials() where currentUsername.length <= 0 || currentUsername == "" || currentUsername == savedUsername {
    ...
    func doLogin(newLogin newLogin: Bool) {
        ...
        self.animateActivityIndicator(on: true)
