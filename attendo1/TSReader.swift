...
class TSReader {
    ...
    func getGradesForClass(currentClass: Class) -> GradeGroup {
        ...
        for link in classPage.css("a, link") {
            ...
            if theads.underestimateCount() < 1 { return rootGroup }
            let thead = theads[0]
            
            let thArray = thead.css("th")
            for i in 0 ..< thArray.underestimateCount() {
            ...
            for row in gradebookPage.css("tr") {
                ...
                if cols.underestimateCount() < 3 { continue }
                ...
                if (cols[nameIndex ?? 1].toHTML ?? "").containsString("categoryHeading") {
                    ...
                    if let scoreIndex = scoreIndex where scoreIndex < cols.underestimateCount() {
                ...
                //grade in the existing category
                    ...
                    if let commentIndex = commentIndex where commentIndex < cols.underestimateCount() {
